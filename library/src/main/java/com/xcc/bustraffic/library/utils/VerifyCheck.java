...
public class VerifyCheck {
	...
	public static boolean isAccountVerify(String account){
		...
		if(account == null || "".equals(account.trim())) {
			...
			Pattern patternAccount = Pattern.compile("^([a-zA-Z]{1}[a-zA-Z0-9_]{3,14})$");
	...
	/**
	 * 验证密码是否通过
	 * 验证密码是否通过,支持6-16位 大小写英文字母、数字、下划线。
	 */
	...
	/**
	 * 验证验证码码是否通过
	 * 验证密码是否通过,支持4位 大小写英文字母、数字。
	 */
	public static boolean isPicVerify(String picVerifyString){
		if (picVerifyString == null || "".equals(picVerifyString.trim())) {
			return false;
		}else{
			String picVerifyTrim = picVerifyString.trim();
			Pattern patternAccount = Pattern.compile("^([a-zA-Z0-9]{4})$");
			Matcher matcherAccount = patternAccount.matcher(picVerifyTrim);
			if (!matcherAccount.matches()) {
				return false;
			}else{
				return true;
